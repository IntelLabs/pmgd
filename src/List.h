...
namespace Jarvis {
    ...
    // TODO Use a variable allocator when available
    ...
    public:
        List() { _list = NULL; }
        // Use the FixedAllocator (for now) for list element allocations
        ...
        // TODO Could add flags for duplicates and sorting choices
        T* add(const T &value, FixedAllocator &allocator);
        void remove(const T &value, FixedAllocator &allocator);
        T* find(const T &val);
        size_t num_elems() const { return _num_elems; }
    };

    // Function definitions in header too since this is a templated class
    // and defining these methods in .cc file causes linker issues
    template <typename T> T* List<T>::add(const T &value, FixedAllocator &allocator)
    {
        // Special case because of no header node and 
        // sorting constraint
        if (_list == NULL || value < _list->value) {
            ListType *temp1 = (ListType *)allocator.alloc();
            temp1->value = value;
            temp1->next = _list;
            // TODO log
            _list = temp1;
